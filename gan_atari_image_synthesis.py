...
class Discriminator(nn.Module):
    ...
    def forward(self, x):
        ...
        # Discriminator Model: x.shape=torch.Size([64, 3, 128, 128])	res.shape=torch.Size([64, 1, 1, 1])	final_res.shape=torch.Size([64])
...
def train_ignite(envs: List[gym.Env], device: torch.device) -> None:
    image_shape = envs[0].observation_space.shape
    print("\nImage Shape: ", image_shape)
    print("\nList of", len(envs), " Environments:-")
    for idx in range(len(envs)):
    ...
    # for batch_images in iterate_batches(envs): => Simulated by `engine.run(data=iterate_batches(env))`
    # where `data=batch_images` that we get iteratively via generator.
    def process_batch(trainer, batch_images) -> Tuple[float, float]:
        # The next 3 steps essentially define the training steps of a Generative Adversarial Network (GAN).
        # Step 1: Generate fake images from a random vector using the Generator network.
        rand_generator_input = torch.FloatTensor(BATCH_SIZE, LATENT_VEC_SIZE, 1, 1)
        rand_generator_input.normal_(0,1)
        ...
        if DEBUG_STEPS:
            print("num_iters =", trainer.state.iteration, "\tdiscriminator loss =", disc_loss.item(), "\tgenerator_loss =", gen_loss.item())

        if trainer.state.iteration % num_steps_save_images == 0:
            fake_images_grid = visionutils.make_grid(fake_gen_images.data[:64], nrow=NUM_IMAGES_GRID_ROW, padding=4, normalize=True)
            trainer.tb.writer.add_image("Generated Images", fake_images_grid, trainer.state.iteration)

            real_images_grid = visionutils.make_grid(batch_images.data[:64], nrow=NUM_IMAGES_GRID_ROW, padding=4, normalize=True)
            trainer.tb.writer.add_image("Real Images", real_images_grid, trainer.state.iteration)

            trainer.tb.writer.flush()
            print(f"\nSaved GAN generated and real images in tensorboard at iteration number {trainer.state.iteration}\n")

        return disc_loss.item(), gen_loss.item()

    engine = Engine(process_batch)
    tb = tb_logger.TensorboardLogger(logdir=None)
    engine.tb = tb

    gen_loss_chart_name = "Avg Generator Loss"
    RunningAverage(output_transform=lambda out: out[1]).attach(engine, gen_loss_chart_name)
    dis_loss_chart_name = "Avg Discriminator Loss"
    RunningAverage(output_transform=lambda out: out[0]).attach(engine, dis_loss_chart_name)

    handler = tb_logger.OutputHandler(tag="Train GAN", metric_names=[gen_loss_chart_name, dis_loss_chart_name])
    ...
    def log_losses(trainer):
        if trainer.state.iteration % num_steps_write_report == 0:
            curr_time = timer.value()
            elapsed_time_hrs, elapsed_time_mins, elapsed_time_secs = get_elapsed_time(start_time, curr_time)
            print("Trained the GAN for Atari games image synthesis for %d iterations in %d hours, %d mins, and %d secs: Generator_Loss=%.6f  Discriminator_Loss=%.6f" % \
                    (trainer.state.iteration, elapsed_time_hrs, elapsed_time_mins, elapsed_time_secs, \
                     trainer.state.metrics[gen_loss_chart_name], trainer.state.metrics[dis_loss_chart_name]))

    @engine.on(Events.ITERATION_COMPLETED(once=total_max_num_iterations))
    def terminate_training(trainer):
        curr_time = timer.value()
        elapsed_time_hrs, elapsed_time_mins, elapsed_time_secs = get_elapsed_time(start_time, curr_time)
        print("\nTrained the Generative Adversarial Network (GAN) for Atari games image synthesis for %d iterations in %d hours, %d mins, and %d secs: Final Generator_Loss=%.6f  Discriminator_Loss=%.6f" \
            % (trainer.state.iteration, elapsed_time_hrs, elapsed_time_mins, elapsed_time_secs, \
               trainer.state.metrics[gen_loss_chart_name], trainer.state.metrics[dis_loss_chart_name]))
        trainer.terminate()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_ignite", action="store_true", help="Use PyTorch Ignite to train the GAN for generating synthetic Atari games images.")
    ...
    if args.use_ignite:
        print("\nTraining the GAN using PyTorch Ignite lib ...")
        train_ignite(envs, device)
    else:
